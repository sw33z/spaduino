...
public:
  ...
  T &get(int const address, T &t) {
    ...
    memcpy(static_cast<uint8_t *>(&t), _data + address, sizeof(T));
    return t;
  }
  ...
  const T &put(int const address, const T &t) {
    if (address < 0 || address + sizeof(T) > _size)
      return t;
    if (memcmp(_data + address, static_cast<const uint8_t *>(&t), sizeof(T)) != 0) {
      _dirty = true;
      memcpy(_data + address, static_cast<const uint8_t *>(&t), sizeof(T));
    }
